...
from .filters import CourseFilter, EntryFilter, UniversityFilter, CutOffFilter


def search(request):
    course_list = Course.objects.all()
    course_filter = CourseFilter(request.GET, queryset=course_list)
    return render(request, 'search/courses.html', {'filter': course_filter})


def entries(request):
    entry_list = Entry.objects.all()
    entry_filter = EntryFilter(request.GET, queryset=entry_list)
    # cut_off_filter = CutOffFilter(request.GET, queryset=entry_list)
    context = {
        'filter': entry_filter,
...
def universities(request):
    ...
    university_filter = UniversityFilter(request.GET, queryset=university_list)
    return render(request, 'search/universities.html', {'filter': university_filter})
...
def consult(request):
    ...
    if request.GET:
        ...
        # math, geo / bio, chem / phy
            ...
            message = "conventional related careers"
            ...
            message = " No career type matching "
...
# to do
# the general answer for social, enterp, convec
# by sunday
